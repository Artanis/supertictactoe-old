...
var STTT = function () {
    ...
    var game_is_ready = function () {
        return last_game_state !== null && channel_is_ready();
    };
    ...
    // callbacks
    var update_game_state_cb = function (game_state) {
        ...
        if (last_game_state.winner !== null) {
            update_status("Player " + winner + "has won");
        }
    };
...
//            age.append(document.createTextNode(created.toString()));
    ...
    var open_channel_cb = function (data) {
        ...
        socket.onmessage = function(m) {
            ...
            switch(msg.mode) {
                ...
                case "start-game":
                    $("#GameBoard *").remove();
                    $("#GameBoard").append(build_html_supertictactoe_board({id:"SuperTicTacToe"}));
                    $("#CurrentGameID").attr("value", game_id);
                    game_id = msg.game_id;
                    client_player = msg.player;
                    update_status("Game started", 10000);
    ...
    var join_game = function(game_id) {
        update_status("Join existing game");
        open_channel();
